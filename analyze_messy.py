...
def main():
    try:
        with open('llm_messy_scores.json', 'r', encoding='utf-8') as f:
            judge_data = json.load(f)
        with open('messy_human_scores.json', 'r', encoding='utf-8') as f:
    ...
    print('\n--- Validation Pipeline Analysis (Messy Batch) ---')
    ...
    if disagreements:
        ...
        for d in sorted(disagreements, key=lambda x: x['diff'], reverse=True):
            ...
            print(f"LLM Says: {justification}")
    else:
